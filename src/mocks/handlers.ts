...
export const handlers = [
  ...
  // Users
  http.post<never, RegistrationRequest>(
    "/users/registration",
    async ({ request }) => {
  ...
  http.put<never, JobTransferRequest>(
    "/api/job/transfer",
    async ({ request }) => {
      const user = authenticateUser(request);
      if (!user || user.role !== "admin") {
  ...
  // Applications
  http.post<never, ApplicationRequest>(
    "/api/application",
    async ({ request }) => {
      const user = authenticateUser(request);
  ...
  http.get("/api/application", ({ request }) => {
    ...
    const items = parseInt(
      new URL(request.url).searchParams.get("items") || "10"
    );
    const startIndex = (page - 1) * items;
    const endIndex = startIndex + items;
  ...
  http.get<{ jobId: string }>(
    ...
    ({ params, request }) => {
      ...
      const { jobId } = params;
      const job = jobs.find(
        (j) => j.id === parseInt(jobId) && j.userId === user.id
      );
      if (!job) {
        return HttpResponse.json(
  ...
  http.get("/api/user/manager/:id", ({ params, request }) => {
    ...
    const manager = users.find(
      (u) => u.id === managerId && u.role === "hiring-manager"
    );
  ...
  http.get("/api/stats/applicant", ({ request }) => {
    ...
    return HttpResponse.json({
      ...
      reviewedApplications: userApplications.filter(
  ...
  http.get("/api/stats/manager", ({ request }) => {
    ...
    return HttpResponse.json({
      ...
      openJobs: managerJobs.filter((job) => job.listingStatus === "open")
        .length,
      closedJobs: managerJobs.filter((job) => job.listingStatus === "closed")
        .length,
      totalApplications: jobApplications.length,
      pendingApplications: jobApplications.filter(
        (app) => app.applicationStatus === "pending"
      ).length,
      reviewedApplications: jobApplications.filter(
        (app) => app.applicationStatus === "reviewed"
      ).length,
